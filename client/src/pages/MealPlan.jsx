      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold gradient-text">
            ...
            {dayIndex !== undefined && (
              <span className="text-text-muted font-normal text-sm ml-1">· {DAYS[dayIndex]}</span>
            )}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3" noValidate>
          {mode === 'recipe' ? (
            ...
            <div>
              ...
              <input
                className={`input ${error ? 'border-red-500/60' : ''}`}
                placeholder="Protein shake, Leftovers…"
                value={customName}
                maxLength={100}
    ...
    <motion.div
      ...
      className={`relative group rounded-xl border overflow-hidden transition-all duration-200
        ${meal.isCooked
          ...
          : 'glass border-white/10 hover:border-brand-500/30'}`}
    >
      ...
      {meal.isCustom && (
        <div className="h-8 flex items-center justify-center text-lg bg-white/5">✏️</div>
      )}

      <div className="p-2">
        <p className={`text-xs font-medium leading-snug line-clamp-2 ${
          meal.isCooked ? 'text-brand-400 line-through' : 'text-text-primary'
        }`}>
          {displayTitle || '—'}
        </p>
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between
        ...
        <button
          ...
          className={`text-xs transition-colors ${
            meal.isCooked ? 'text-brand-400' : 'text-text-muted hover:text-brand-400'
          }`}
        >
          {meal.isCooked ? '✅' : '⬜'}
        </button>
...
export default function MealPlan() {
  ...
  const handleDeleteClick = () => {
    if (confirmDelete) {
      // Second click — actually delete (optimistic, UI updates instantly)
      deleteMealPlan();
  ...
  return (
    <main className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          {mealPlan && (
            ...
            >
              ...
              {confirmDelete ? 'Tap again to confirm' : 'Delete plan'}
            </motion.button>
          )}
      ...
      {/* Content — AnimatePresence ensures smooth transition between plan and empty state */}
            ...
            <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
              <div className="min-w-[600px]">
                ...
                <div className="grid grid-cols-7 gap-2 mb-3">
                  {DAYS.map((d) => (
                    <div key={d}
                      className="text-center text-xs font-semibold text-text-muted uppercase tracking-wide py-1">
                      {d}
                    </div>
                  ))}
                </div>

                {/* Meal rows — only breakfast, lunch, dinner */}
