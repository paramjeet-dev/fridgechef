...
export default function Inventory() {
  const { inventory, grouped, total, isLoading, isSubmitting, fetchInventory, bulkDelete, searchQuery, setSearch } = useInventoryStore();
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Remove ${selectedIds.size} item${selectedIds.size !== 1 ? 's' : ''}?`)) return;
    await bulkDelete([...selectedIds]);
    clearSelection();
  };

  const sections = Object.entries(grouped).filter(([, items]) => items.length > 0).sort(([a], [b]) => a.localeCompare(b));
  ...
  return (
    <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 relative z-10">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            ...
            {expiringCount > 0 && <span className="ml-2 text-amber-400 font-medium">· {expiringCount} expiring soon ⚠️</span>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => setShowAddModal(true)} className="btn-primary text-sm flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="relative mb-6">
        ...
        <input className="input pl-10" placeholder="Search ingredients…" value={searchQuery} onChange={(e) => setSearch(e.target.value)} />
        {searchQuery && (
        ...
        {selectedIds.size > 0 && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <div className="flex gap-2">
              <button onClick={clearSelection} className="text-xs text-text-muted hover:text-text-primary font-medium">Clear</button>
      ...
      {/* Content */}
      {isLoading ? (
        <div className="flex justify-center py-20"><LoadingSpinner size="lg" label="Loading fridge…" /></div>
      ) : inventory.length === 0 ? (
        <EmptyState icon="🧊" title={searchQuery ? 'No matching items' : 'Your fridge is empty'}
          description={searchQuery ? `No ingredients match "${searchQuery}".` : 'Add items manually or scan your fridge.'}
          action={!searchQuery && <button onClick={() => setShowAddModal(true)} className="btn-primary">+ Add your first item</button>} />
      ) : (
        <>
          {inventory.length > 1 && (
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs text-text-muted">{inventory.length} items</span>
              <button onClick={selectedIds.size === inventory.length ? clearSelection : selectAll}
                  ...
                  <AnimatePresence mode="popLayout">
                    {items.map((item) => (
                      <div key={item._id} className="relative">
                        <input type="checkbox"
                          className="absolute top-3 left-3 z-10 w-4 h-4 accent-violet-500 cursor-pointer"
                          checked={selectedIds.has(item._id)}
                          onChange={() => toggleSelect(item._id)}
                          aria-label={`Select ${item.displayName}`} />
                        <div className={selectedIds.has(item._id) ? 'ring-2 ring-brand-500/50 rounded-2xl' : ''}>
